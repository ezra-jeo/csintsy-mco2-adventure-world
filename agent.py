    ...
    def query_move(self, player_pos):
        try:
            ...
            if tile_type == "safe":
                ...
                # if player_pos in self.map.glitter_pos:
                #     list(self.prolog.query(f"move({R}, {C}, [glitter], {self.N})."))
                else:
                ...
                if tile_type != "pit":
                    ...
                    # if player_pos in self.map.glitter_pos:
                    #     list(self.prolog.query(f"move({R}, {C}, [{tile_type}, glitter], {self.N})."))
                    else:
                        list(self.prolog.query(f"move({R}, {C}, [{tile_type}], {self.N})."))
                
                    
            # print(list(self.prolog.query("unexplored_safe(R, C).")))
            # print(list(self.prolog.query("explored_safe(R, C).")))
            # print(list(self.prolog.query("unknown(R, C).")))
